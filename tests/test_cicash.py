...
from cicash import Ledger, ci, Denied, AuditBroken, ESCALATE, REPLAN, RETRY_AFTER
...
class TestHappyPath(unittest.TestCase):
    def test_pay_and_balance(self):
        ...
        self.assertEqual(w.balance()["available"], ci(50))
        r = w.pay(api.quote(ci(2), "research"), idem_key="run1/step1")
        self.assertEqual(r.amount, ci(2))
        self.assertEqual(w.balance()["available"], ci(48))

    def test_settle_less_than_quoted(self):
        clk, led, p, api, w = setup()
        q = api.quote(ci(5), "research")
        h = w.authorize(q, "k1")
        r = w.settle(h, actual=ci(3), idem_key="k2")
        self.assertEqual(r.amount, ci(3))
        self.assertEqual(w.balance()["available"], ci(47))

    def test_settle_more_than_quoted_refused(self):
        clk, led, p, api, w = setup()
        h = w.authorize(api.quote(ci(5), "research"), "k1")
        with self.assertRaises(Denied) as e:
            w.settle(h, actual=ci(6), idem_key="k2")
    ...
    def test_hold_reserves_funds_then_expires(self):
        ...
        self.assertEqual(w.balance()["available"], ci(40))   # held, not spent
        clk.advance(61)
        self.assertEqual(w.balance()["available"], ci(50))   # auto-released


class TestRetriesAreFree(unittest.TestCase):
    def test_same_idem_key_charges_once(self):
        clk, led, p, api, w = setup()
        q = api.quote(ci(4), "research")
...
class TestAttenuationIsEconomic(unittest.TestCase):
    ...
    def test_cannot_escape_parent_cap_by_forking_children(self):
        clk, led, p, api, w = setup(budget=ci(10))
        a = w.delegate(budget=ci(10))
        b = w.delegate(budget=ci(10))
        a.pay(api.quote(ci(6), "research"), idem_key="a1")
        with self.assertRaises(Denied) as e:
            b.pay(api.quote(ci(6), "research"), idem_key="b1")
    ...
    def test_deep_chain_still_bound(self):
        ...
        cur = w
        for _ in range(5):
            cur = cur.delegate(budget=ci(10))
        self.assertEqual(cur.token.depth, 5)
        cur.pay(api.quote(ci(9), "research"), idem_key="d1")
        with self.assertRaises(Denied):
            cur.pay(api.quote(ci(2), "research"), idem_key="d2")

    def test_widening_refused_loudly(self):
        clk, led, p, api, w = setup(budget=ci(50), per_tx=ci(5))
        with self.assertRaises(Denied) as e:
            w.delegate(budget=ci(500))
        self.assertEqual(e.exception.reason, "WIDENING_REFUSED")
        with self.assertRaises(Denied):
            w.delegate(budget=ci(5), payees=["attacker.xyz"])
    ...
    def test_widening_would_be_inert_even_if_forced(self):
        ...
        from cicash.ledger import Wallet
        forced = Wallet(led, w.token.attenuate([ser(MAX_TOTAL, ci(9999))]), w.signer)
        with self.assertRaises(Denied) as e:
            forced.pay(api.quote(ci(50), "research"), idem_key="x1")
        self.assertEqual(e.exception.reason, "TOTAL_EXHAUSTED")


class TestTokenIntegrity(unittest.TestCase):
    def test_removing_a_caveat_breaks_signature(self):
        clk, led, p, api, w = setup(budget=ci(10), per_tx=ci(1))
        stripped = Token(
        ...
        with self.assertRaises(Denied) as e:
            bad.pay(api.quote(ci(5), "research"), idem_key="t1")
    ...
    def test_leaked_token_without_secret_is_worthless(self):
        ...
        with self.assertRaises(Denied) as e:
            led.authorize(leaked, "deadbeef", api.quote(ci(1), "research"), "atk")
    ...
    def test_pop_is_bound_to_the_request(self):
        ...
        clk, led, p, api, w = setup()
        q1 = api.quote(ci(1), "research")
        q2 = api.quote(ci(9), "research")
        pop = w._pop(q1, "k")
        with self.assertRaises(Denied) as e:
...
class TestInjectionDefence(unittest.TestCase):
    def test_payee_allowlist_blocks_injected_recipient(self):
        ...
        q = attacker.quote(ci(900), "research")          # agent was talked into this
        with self.assertRaises(Denied) as e:
    ...
    def test_purpose_allowlist(self):
        clk, led, p, api, w = setup()
        with self.assertRaises(Denied) as e:
            w.pay(api.quote(ci(1), "buy_gift_cards"), idem_key="inj2")
        self.assertEqual(e.exception.reason, "PURPOSE_NOT_ALLOWED")

    def test_agent_cannot_invent_a_price(self):
        clk, led, p, api, w = setup(per_tx=ci(5))
        with self.assertRaises(Denied) as e:
            w.pay(api.quote(ci(20), "research"), idem_key="inj3")
        self.assertEqual(e.exception.reason, "PER_TX_EXCEEDED")

    def test_forged_quote_rejected(self):
        clk, led, p, api, w = setup()
        q = api.quote(ci(1), "research")
        tampered = q.__class__(q.quote_id, q.payee, ci(500), q.purpose,
                               q.expires_at, q.sig)
        with self.assertRaises(Denied) as e:
    ...
    def test_stale_quote_rejected(self):
        clk, led, p, api, w = setup()
        q = api.quote(ci(1), "research", ttl_s=30)
        clk.advance(31)
        with self.assertRaises(Denied) as e:
...
class TestRunawayLoop(unittest.TestCase):
    def test_rate_cap_stops_the_loop_and_says_when_to_retry(self):
        ...
        for i in range(3):
            w.pay(api.quote(ci(1), "research"), idem_key=f"loop{i}")
        with self.assertRaises(Denied) as e:
            w.pay(api.quote(ci(1), "research"), idem_key="loop3")
        ...
        w.pay(api.quote(ci(1), "research"), idem_key="loop4")

    def test_amount_rate_cap(self):
        clk, led, p, api, w = setup(rate={"max_count": None, "max_amount": ci(5), "window_s": 60})
        w.pay(api.quote(ci(4), "research"), idem_key="r1")
        with self.assertRaises(Denied) as e:
            w.pay(api.quote(ci(2), "research"), idem_key="r2")
        self.assertEqual(e.exception.reason, "RATE_LIMITED")
...
class TestRevocation(unittest.TestCase):
    def test_revoking_parent_kills_descendants_instantly(self):
        clk, led, p, api, w = setup()
        a = w.delegate(budget=ci(10))
        b = a.delegate(budget=ci(5))
        b.pay(api.quote(ci(1), "research"), idem_key="ok1")
        p.revoke(w)
        for wallet in (w, a, b):
            with self.assertRaises(Denied) as e:
                wallet.pay(api.quote(ci(1), "research"), idem_key="after" + wallet.token.token_id)
    ...
    def test_expiry(self):
        ...
        clk.advance(3601)
        with self.assertRaises(Denied) as e:
            w.pay(api.quote(ci(1), "research"), idem_key="exp1")
...
class TestAudit(unittest.TestCase):
    def test_chain_verifies_and_detects_tampering(self):
        clk, led, p, api, w = setup()
        sub = w.delegate(budget=ci(10), note="sub")
        w.pay(api.quote(ci(1), "research"), idem_key="a1")
        sub.pay(api.quote(ci(2), "research"), idem_key="a2")
    ...
    def test_receipt_binds_purpose_and_lineage(self):
        clk, led, p, api, w = setup()
        sub = w.delegate(budget=ci(10))
        r = sub.pay(api.quote(ci(2), "research"), idem_key="p1")
...
class TestPlannerErgonomics(unittest.TestCase):
    def test_denial_tells_the_agent_what_to_do(self):
        clk, led, p, api, w = setup(budget=ci(3))
        with self.assertRaises(Denied) as e:
            w.pay(api.quote(ci(10), "research"), idem_key="e1")
        d = e.exception.as_dict()
        self.assertIn("action", d)
        self.assertIn("hint", d)
        self.assertEqual(d["remaining"], ci(3))

    def test_can_afford_before_acting(self):
        clk, led, p, api, w = setup(budget=ci(3), per_tx=ci(2))
        self.assertTrue(w.can_afford(ci(2)))
        self.assertFalse(w.can_afford(ci(3)))   # blocked by per-tx, not total
