...
python3 -m agentcash.cli --db "$DB" grant \
...
python3 -m agentcash.cli --db "$DB" balance --wallet "$W"

# 3. point any MCP client at it (see examples/mcp_config.json)
echo
echo "MCP:  AGENTCASH_DB=$DB AGENTCASH_WALLET=$W python3 -m agentcash.mcp_server"
echo "HTTP: python3 -m agentcash.cli --db $DB serve"
echo

# 4. the kill switch, whenever you want it
echo "revoke with:  python3 -m agentcash.cli --db $DB revoke --wallet $W"
