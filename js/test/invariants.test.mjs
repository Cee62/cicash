...
import { Ledger, Wallet, usd, Denied, crypto } from "../src/index.js";
...
function setup(opts = {}) {
  ...
  const w = p.grant({
    budget: usd(50), payees: ["api.search"], purposes: ["research"], ...opts,
  });
...
test("happy path and balance", () => {
  const { api, w } = setup();
  assert.equal(w.balance().available, usd(50));
  const r = w.pay(api.quote(usd(2), "research"), "run1/step1");
  assert.equal(r.amount, usd(2));
  assert.equal(w.balance().available, usd(48));
});

test("retries are free", () => {
  const { led, api, w } = setup();
  const q = api.quote(usd(4), "research");
  ...
  assert.equal(w.balance().available, usd(46));
});

test("a child cannot escape the parent cap by forking siblings", () => {
  const { api, w } = setup({ budget: usd(10) });
  const a = w.delegate({ budget: usd(10) });
  const b = w.delegate({ budget: usd(10) });
  a.pay(api.quote(usd(6), "research"), "a1");
  assert.throws(() => b.pay(api.quote(usd(6), "research"), "b1"), e => {
...
test("delegation is a ratchet", () => {
  const { w } = setup({ perTx: usd(5) });
  assert.throws(() => w.delegate({ budget: usd(500) }), /WIDENING_REFUSED/);
  assert.throws(() => w.delegate({ budget: usd(5), payees: ["attacker.xyz"] }),
    /WIDENING_REFUSED/);
});

test("a widening caveat forced past delegate() is still inert", () => {
  const { led, api, w } = setup({ budget: usd(10) });
  const forced = new Wallet(led,
    w.token.attenuate(['max_total:' + usd(9999)]), w.signer);
  assert.throws(() => forced.pay(api.quote(usd(50), "research"), "x1"),
    /TOTAL_EXHAUSTED/);
});
...
test("a leaked token without its key buys nothing", () => {
  ...
  assert.throws(() => led.authorize(w.token, "deadbeef",
    api.quote(usd(1), "research"), "atk"), e => {
...
test("a captured proof cannot be re-aimed at another payment", () => {
  const { led, api, w } = setup();
  const q1 = api.quote(usd(1), "research");
  const q2 = api.quote(usd(9), "research");
...
test("injected payee is refused with a REPLAN action", () => {
  ...
  assert.throws(() => w.pay(attacker.quote(usd(900), "research"), "inj1"), e => {
...
test("rate cap stops a loop and says when to retry", () => {
  ...
  for (let i = 0; i < 3; i++) w.pay(api.quote(usd(1), "research"), "loop" + i);
  let retryAfter = null;
  assert.throws(() => w.pay(api.quote(usd(1), "research"), "loop3"), e => {
  ...
  w.pay(api.quote(usd(1), "research"), "loop4");
});

test("revoking a parent kills the whole subtree", () => {
  const { p, api, w } = setup();
  const a = w.delegate({ budget: usd(10) });
  const b = a.delegate({ budget: usd(5) });
  b.pay(api.quote(usd(1), "research"), "ok1");
  p.revoke(w);
  for (const wallet of [w, a, b]) {
    assert.throws(() => wallet.pay(api.quote(usd(1), "research"),
...
test("holds reserve funds and expire", () => {
  ...
  assert.equal(w.balance().available, usd(40));
  clock.advance(61);
  assert.equal(w.balance().available, usd(50));
});
...
test("receipt chain verifies and detects tampering", () => {
  const { led, api, w } = setup();
  const sub = w.delegate({ budget: usd(10) });
  w.pay(api.quote(usd(1), "research"), "a1");
  sub.pay(api.quote(usd(2), "research"), "a2");
